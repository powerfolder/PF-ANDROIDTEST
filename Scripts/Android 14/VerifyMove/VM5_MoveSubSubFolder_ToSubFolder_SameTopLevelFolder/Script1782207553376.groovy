...
String secSubFolderName = 'SecSubFolder_' + timestamp_folder

Mobile.setText(findTestObject('Folder_Menu/EnterNewFolderName'),secSubFolderName, 30)
Mobile.delay(2)
Mobile.tap(findTestObject('Folder_Menu/ClickOnOkButton'), 30)

// wait some seconds after setting up second sub folder
Mobile.delay(3)

// verifying second sub folder is existing
TestObject second_sub_folder_obj = new TestObject()
second_sub_folder_obj.addProperty("xpath", ConditionType.EQUALS, "//*[@text='" + secSubFolderName + "']")
Mobile.verifyElementExist(second_sub_folder_obj, 5)
...
// create sub folder name based on timestamp
...
// wait some seconds after setting up sub folder
Mobile.delay(3)

// verifying sub folder is existing
...
// create subsub folder name based on timestamp
String subsubFolderName = 'subSubFolder_' + timestamp_folder
Mobile.setText(findTestObject('Folder_Menu/EnterNewFolderName'), subsubFolderName, 30)
Mobile.delay(2)
Mobile.tap(findTestObject('Folder_Menu/ClickOnOkButton'), 30)

// wait some seconds after setting up subsub folder
Mobile.delay(3)

// verifying subsub folder is existing
TestObject subsub_folder_obj = new TestObject()
subsub_folder_obj.addProperty("xpath", ConditionType.EQUALS, "//*[@text='" + subsubFolderName + "']")
Mobile.verifyElementExist(subsub_folder_obj, 5)

// Swap left to right for move target folder
CustomKeywords.'utils.SwipeLeftToRight.swipe'(subsub_folder_obj)
...
// Tap to open second sub folder
Mobile.tap(second_sub_folder_obj, 5)

// Tap on move button
Mobile.tap(findTestObject('SwipeMove/MoveButton'), 30)
Mobile.delay(5)

// Verify subsub folder after move on target folder
Mobile.verifyElementExist(subsub_folder_obj, 5)
