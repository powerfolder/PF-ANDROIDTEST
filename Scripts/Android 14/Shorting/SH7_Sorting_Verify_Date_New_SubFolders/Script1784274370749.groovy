...
Mobile.tap(findTestObject('Shorting/ShortByDateNew'), 30)
Mobile.delay(3)
...
if (firstTime.contains("just now")) {
	...
	assert matcher.find() : "Cannot extract minutes from: ${firstTime}"
...
if (secondTime.contains("just now")) {
	...
	assert matcher.find() : "Cannot extract minutes from: ${secondTime}"
	secondValue = matcher.group(1).toInteger()
}

println("First : ${firstTime} -> ${firstValue}")
println("Second: ${secondTime} -> ${secondValue}")

// Date New
assert firstValue < secondValue :
	"Date New sorting failed"*/
