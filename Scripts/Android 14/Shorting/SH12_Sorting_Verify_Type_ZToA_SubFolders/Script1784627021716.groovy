...
// Select A-Z sorting
Mobile.tap(findTestObject('Object Repository/Shorting/ShortByTypeZToA'), 30)

// Wait for sorting to complete
Mobile.delay(2)

// ==========================================
// Verify Z-A File Sorting
// ==========================================
// Get Android driver
...
// Collect only uploaded Screenshot files
for (WebElement element : elements) {

	String text = element.getText()

	if (text != null && text.startsWith("Screenshot")) {
		fileNames.add(text)
	}
}
...
// Verify at least two uploaded files are displayed
assert fileNames.size() >= 2 : "Expected at least 2 uploaded files, but found ${fileNames.size()}"

// Create expected Z-A order
List<String> expected = new ArrayList<>(fileNames)
Collections.sort(expected)
Collections.reverse(expected)

// Verify Z-A sorting
assert fileNames == expected :
		"Z-A File Sorting failed.\nExpected: ${expected}\nActual: ${fileNames}"

println("Z-A File Sorting verified successfully.")
