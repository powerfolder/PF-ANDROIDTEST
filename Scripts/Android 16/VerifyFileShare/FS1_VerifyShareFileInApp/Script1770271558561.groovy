...
// tap on save-button
tapOnFolder(sharedFolder)
Mobile.tap(findTestObject('ShareFile/SaveButton'),30)
Mobile.delay(5)

// startup app after starting
Mobile.startExistingApplication('de.goddchen.android.powerfolder.A', FailureHandling.STOP_ON_FAILURE)
Mobile.delay(15)

// got to shared folder
tapOnFolder(sharedFolder)

// verifying file upload (filename needs to be screenshot.*)
TestObject sharedFile_obj = new TestObject()
sharedFile_obj.addProperty("xpath", ConditionType.EQUALS, "//*[contains(@text, 'Screenshot')]")
Mobile.verifyElementExist(sharedFile_obj, 5)

// verify shared file on server
String base = GlobalVariable.WebdavURL
String user = GlobalVariable.userid
String pass = GlobalVariable.password

String fileName = Mobile.getAttribute(file_obj, 'text', 5)
String serverPath = sharedFolder + '/' + fileName
