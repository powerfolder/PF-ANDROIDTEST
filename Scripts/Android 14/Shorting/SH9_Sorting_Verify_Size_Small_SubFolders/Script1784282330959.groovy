...
/*
// ==========================================
// Verify Small Size Sorting
// ==========================================

AndroidDriver driver = MobileDriverFactory.getDriver()

List<WebElement> sizeElements = driver.findElements(
	By.xpath("//android.widget.ListView/android.view.View/android.widget.TextView[2]")
)

println("Total Size Elements : " + sizeElements.size())
...
for (WebElement element : sizeElements) {
	...
	println("Size Text : " + text)

	if (text == null || text.isEmpty())
		continue

	def matcher = text =~ /(\\d+(\\.\\d+)?)\\s*(KB|MB)/

	if (matcher.find()) {

		double size = matcher.group(1).toDouble()

		if (matcher.group(3).equalsIgnoreCase("MB")) {
			size *= 1024
		}
...
println("Detected Sizes : " + sizes)

assert sizes.size() == 2 :
	   "Expected 2 file sizes but found ${sizes.size()}"

// Verify Small Size Sorting
for (int i = 0; i < sizes.size() - 1; i++) {

	assert sizes.get(i) <= sizes.get(i + 1) :
		"Small Size sorting failed. ${sizes.get(i)} KB appears before ${sizes.get(i + 1)} KB"
}

println("Small Size Sorting verified successfully.")*/
