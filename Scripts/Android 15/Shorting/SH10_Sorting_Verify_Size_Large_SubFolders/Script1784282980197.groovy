...
// Select small size sorting option
Mobile.tap(findTestObject('Object Repository/Shorting/SmallSizeShortingOption'), 30)
...
/*
// ==========================================
// Verify Smallest Size Sorting
// ==========================================

// Get Android driver
AndroidDriver driver = MobileDriverFactory.getDriver()

// Find all size labels (e.g. "25.23 KB", "2.5 MB")
List<WebElement> sizeElements = driver.findElements(
		By.xpath("//android.widget.TextView[matches(@text, '.*(KB|MB|GB).*')]")
)

assert sizeElements.size() >= 2 : "Less than 2 file sizes found."

List<Double> sizesInKB = []

for (WebElement e : sizeElements) {

	String sizeText = e.getText().trim()
	println("Found Size : " + sizeText)
	...
	switch(unit) {
		...
		case "MB":
			sizesInKB.add(value * 1024)
			break

		case "GB":
			sizesInKB.add(value * 1024 * 1024)
			break
	}
}

println("Sizes in KB : " + sizesInKB)

assert sizesInKB.get(0) >= sizesInKB.get(1) :
"Large Size sorting failed. Actual order: ${sizesInKB}"

println("Large Size sorting verified successfully.")
*/
// go to home - toplvl
